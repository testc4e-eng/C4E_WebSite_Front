...
function getSortedCandidatures(candidatures: Candidature[], sortBy: string, sortOrder: 'asc' | 'desc') {
  ...
  const fetchCandidatures = async () => {
    try {
      ...
      if (activeTab === 'candidatures') {
        ...
        // Debug détaillé
        const stageSpontaneCount = data.filter(c => c.type === 'stage_spontane').length;
        const spontaneeCount = data.filter(c => c.type === 'spontanee').length;
        console.log(`   - Stage spontané: ${stageSpontaneCount}`);
        console.log(`   - Spontané général: ${spontaneeCount}`);
        
        data.forEach((c, i) => {
          console.log(`   ${i+1}. id=${c.id}, type=${c.type}, nom=${c.nom}, poste=${c.poste}`);
        });
        ...
        console.log(`📊 Après filtrage (emploi/stage/pfe): ${candidaturesSurOffres.length}`);
        
        // Debug détaillé du filtrage
        const emploiCount = candidaturesSurOffres.filter(c => c.type === 'emploi').length;
        const stageCount = candidaturesSurOffres.filter(c => c.type === 'stage').length;
        const pfeCount = candidaturesSurOffres.filter(c => c.type === 'pfe').length;
...
const supprimerCandidature = async (candidature: Candidature) => {
  ...
  const candidaturesFiltreesArchive = archiveFilter === 'tous' 
    ...
    : candidaturesArchivees.filter(c => 
        archiveFilter === 'acceptees' ? c.statut === 'acceptee' : c.statut === 'refusee'
      );
        ...
        {loadingOffres ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {offresFiltrees.map((offre) => {
              ...
              return (
                <div key={offre.id} className="bg-white rounded-xl shadow-lg border border-gray-200 hover:shadow-xl transition-all duration-200">
                  <div className="p-6">
                    ...
                    <div className="flex space-x-2">
                      ...
                      >
                        ...
                        <span>
                          {stats.total === 0 ? 'Aucune candidature' : `Voir (${stats.total})`}
                        </span>
                      </button>
                      <button
...
const getCandidaturesPourOffre = () => {
  ...
  console.log(`🔍 DEBUG - Candidatures après filtrage type (${filtreType}):`, candidaturesFiltreesParType.length);

  // CORRECTION : Utiliser offre_id au lieu de offreId
  const candidaturesPourOffre = candidaturesFiltreesParType.filter(cand => {
    // MATCHING DIRECT par offre_id
    if (cand.offre_id === selectedOffre.id) {
      console.log(`✅ Match direct: ${cand.nom} (offre_id: ${cand.offre_id})`);
      return true;
    }

    console.log(`❌ Non match: ${cand.nom} | offre_id:${cand.offre_id} vs selected:${selectedOffre.id} | type:${cand.type} vs selected:${selectedOffre.type}`);
    return false;
  });

  console.log('🔍 DEBUG - Candidatures finales pour offre:', candidaturesPourOffre);
              ...
              <table className="w-full">
                <thead className="bg-gradient-to-r from-gray-50 to-gray-100">
                  <tr>
                    ...
                    <th className="px-6 py-4 text-left font-semibold text-gray-700">Diplôme</th>
                    <th className="px-6 py-4 text-left font-semibold text-gray-700">Score Compétences</th>
                    <th className="px-6 py-4 text-left font-semibold text-gray-700">Expérience</th>
                    <th className="px-6 py-4 text-left font-semibold text-gray-700">Date Soumission</th>
                    ...
                    <tr key={cand.id} className="hover:bg-gray-50 transition-colors duration-200">
                      ...
                      <td className="px-6 py-4 space-x-2">
                        <select
                          value={cand.statut}
                          onChange={(e) =>
                            changerStatut(cand, e.target.value as Candidature['statut'])
                          }
    ...
    return getSortedCandidatures(
      candidatures.filter(c => {
        ...
        if ((c.type === 'stage' || c.type === 'pfe') && selectedOffre) {
          ...
          return posteCandidat?.includes(titreOffre) || titreOffre.includes(posteCandidat || '');
        }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100">
      ...
      <div className="container mx-auto px-6 py-8">
        ...
        {(activeTab === 'candidatures' || activeTab === 'candidatures-postes') && (
          <StatsOverview />
        )}
        ...
        {activeTab === 'offres' && (
          <section className="space-y-6">
            ...
            {loadingOffres && <div className="text-center py-8 text-gray-600">Chargement des offres...</div>}
                ...
                <table className="w-full">
                  <thead className="bg-gradient-to-r from-gray-50 to-gray-100">
                    <tr>
                      ...
                      <th className="px-6 py-4 text-left font-semibold text-gray-700">Salaire</th>
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {offres.map((offre) => (
                      <tr key={offre.id} className="hover:bg-gray-50 transition-colors duration-200">
                        ...
                        <td className="px-6 py-4 text-gray-600">{offre.salaire || 'N/A'}</td>
                        <td className="px-6 py-4 text-gray-600">
          ...
          <section className="space-y-6">
            ...
            <div className="flex flex-col md:flex-row justify-center items-center space-y-4 md:space-y-0 md:space-x-4">
              <select
                value={filterType}
                onChange={(e) =>
                  setFilterType(e.target.value as 'tous' | 'stage_spontane' | 'spontanee')
                }
                className="p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500 bg-white shadow-sm"
              >
            ...
            {loadingCandidatures && <div className="text-center py-8 text-gray-600">Chargement des candidatures spontanées...</div>}

            {['stage_spontane', 'spontanee'].map((type) => {
              if (filterType !== 'tous' && filterType !== type) return null;
              
              const candidaturesByType = type === 'stage_spontane'
                ? candidatures.filter((c) => c.type === 'stage_spontane')
                : candidatures.filter((c) => c.type === 'spontanee');
              
              // DEBUG: Vérifier le filtrage
              console.log(`🔍 DEBUG - Candidatures pour ${type}:`, candidaturesByType);
              ...
              return (
                 <div key={type} className="bg-white rounded-xl shadow-lg overflow-hidden mb-6 w-full max-w-[98vw] mx-auto">
    ...
    {sortedCandidatures.length}
                  ...
                  {sortedCandidatures.length === 0 ? (
                    <div className="text-center py-12 text-gray-500">
                      ...
                      <h4 className="text-lg font-semibold text-gray-700 mb-2">
                        Aucune candidature {type === 'stage_spontane' ? 'spontanée de stage/PFE' : 'spontanée générale'}
                      </h4>
                      <p className="text-gray-500">
                        {type === 'stage_spontane' 
                          ...
                          {sortedCandidatures.map((cand) => (
                            <tr key={cand.id} className="hover:bg-gray-50 transition-colors duration-200">
                              <td className="px-6 py-4 font-medium text-gray-900 capitalize flex items-center space-x-2">
                                ...
                                <span>
                                  {cand.type === 'stage_spontane' ? 'Stage' : 
                                   cand.type === 'spontanee' ? 'Candidature Spontanée' : cand.type}
                                </span>
                              </td>
                              <td className="px-6 py-4 text-gray-900">{cand.nom}</td>
                              <td className="px-6 py-4 text-gray-600">{cand.email}</td>
              ...
              <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-2xl max-h-[90vh] overflow-hidden animate-fadeIn">
                  ...
                  <div className="space-y-3 overflow-y-auto pr-2 max-h-[70vh] custom-scrollbar">
                    ...
                    <p><strong>📋 Type de candidature :</strong> 
        ...
        <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-2xl max-h-[90vh] overflow-hidden animate-fadeIn">
          ...
          <div className="space-y-3 overflow-y-auto pr-2 max-h-[70vh] custom-scrollbar">
            ...
            {selectedCandidature.domaine && (
              <p><strong>🌍 Domaine :</strong> {selectedCandidature.domaine}</p>
            )}
        ...
        {activeTab === 'archives' && (
          <section className="space-y-6">
            <div className="text-center mb-8">
              <h2 className="text-3xl font-bold text-gray-900 mb-2">Archives des Candidatures</h2>
                  ...
                  <div className="flex justify-between items-center mb-4 border-b pb-2">
                    ...
                    <div className="flex items-center space-x-2">
                      <span className={`px-3 py-1 rounded-full text-sm font-medium ${
                        ...
                        {selectedCandidature.statut === 'acceptee' ? 'Acceptée' : 'Refusée'}
                      </span>
                    </div>
                  </div>
